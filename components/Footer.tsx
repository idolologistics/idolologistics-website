  ...
  return (
    <footer className="site-footer">
      <div className="container footer-grid">
        ...
        <div><h2>Contatti</h2><address>{company.address}</address><a href={`tel:${company.phoneHref}`}>{company.phone}</a><a href={`mailto:${company.email}`}>{company.email}</a></div>
        <div><h2>Informazioni</h2><Link href="/privacy">Privacy</Link><Link href="/cookie-policy">Cookie policy</Link><span className="muted">{company.social.placeholder}</span></div>
      </div>
